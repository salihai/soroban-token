...
import { ArrowDownUp, ArrowUpRight, Loader2, RefreshCw, Trash2, Coffee } from "lucide-react"
import { useWalletStore } from "@/lib/stores/wallet-store"
import { formatTokenAmount, truncateAddress, formatDate } from "@/lib/utils"

type Transaction = {
  id: string
  type: "transfer" | "mint" | "burn" | "freeze" | "unfreeze" | "add coffee point"
  from: string | null
  to: string | null
  amount: string | null
  timestamp: number
...
export default function TransactionHistory() {
  ...
  const getTransactionDescription = (tx: Transaction) => {
    switch (tx.type) {
      ...
      case "add coffee point":
        return `Transferred ${formatTokenAmount(tx.amount || "0")} for 1 coffee point`
      default:
